...
export default function DashboardPage({ params: { lang } }) {
  ...
  const [selectedTimeFrame, setSelectedTimeFrame] = useState<DateRange | undefined>({
    from: new Date(),
  ...
  const exportJobs = () => {
    ...
    if (window.innerWidth > 768) {
      filteredJobs = jobs.filter(job => {
        const jobDate = new Date(job.start_slot);
        return jobDate >= (selectedTimeFrame?.from ?? new Date()) && jobDate <= (selectedTimeFrame?.to ?? addDays(new Date(), 14));
      });
    }
  
    const blob = new Blob([JSON.stringify(filteredJobs)], {type: "text/plain;charset=utf-8"});
          ...
          <div className="flex items-center justify-between space-y-2">
            ...
            <div className="flex items-center space-x-2">
              <Search className="hidden md:block"/>
                <CalendarDateRangePicker className="hidden md:block" onSelect={setSelectedTimeFrame}/>
              <Button onClick={exportJobs}>{dict.dashboard.dashboard.export}</Button>
            </div>
          </div>
