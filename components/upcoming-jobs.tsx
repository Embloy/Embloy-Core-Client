          ...
          <AccordionTrigger>
            <div className="flex items-center justify-start">
              <Avatar className="size-9">
                <AvatarImage src={job.employer_image_url || "/avatars/01.png"} alt="Avatar" />
                <AvatarFallback>{job.employer_name?.substring(0, 2)}</AvatarFallback>
              </Avatar>
              <div className="ml-4 space-y-1 text-left">
                <p className="text-sm font-medium leading-none">{job.employer_name}</p>
                <p className="text-sm text-muted-foreground">{job.employer_email}</p>
              </div>
            </div>
          </AccordionTrigger>
          <AccordionContent>
            <div className="ml-4 items-center justify-start space-y-1">
              ...
              {job.job_type && (() => {
                ...
                return (
                  <span className={`items-left inline-flex rounded-full px-3 py-0.5 text-sm font-medium ${colorClasses}`}>
                    {job.job_type}
                  </span>
                );
              })()}
              ...
              <div className="flex items-center space-x-2">
                ...
                <p className="text-sm text-muted-foreground">{formatDate(lang, job.start_slot)}</p>
                <button className="text-sm text-blue-500 underline" onClick={() => exportToCalendar(job.start_slot, job.position ?? job.title ?? job.employer_name ?? job.job_slug)}>{dict.dashboard.upcoming.t.rowActions.calendar}</button>              
              </div>
              <Separator className="bg-muted"/>
              {job.referrer_url && (
                <div className="flex items-center space-x-2">
                  <ExternalLink className="size-5 text-gray-400" />
                  <a href={job.referrer_url} target="_blank" rel="noopener noreferrer" className="text-sm text-blue-500 underline">{dict.dashboard.upcoming.t.columns.employerURL}</a>
                </div>
              )}
            </div>
          </AccordionContent>
